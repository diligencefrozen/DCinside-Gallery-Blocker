...
export function verifyModel(file = modelFile) {
  return verifyArtifact(file, modelIdentity, "분석 모델", "scripts/prepare-detector.mjs --from <model.onnx 경로>로 먼저 등록해 주세요.");
}

export function verifyTokenizer(file = tokenizerFile) {
  return verifyArtifact(file, tokenizerIdentity, "분석 토크나이저", "학습 결과의 tokenizer.json을 모델과 함께 등록해 주세요.");
}
