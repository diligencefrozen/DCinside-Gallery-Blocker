...
- `일반 댓글 숨기기`
- `이미지 댓글 숨기기`
- `디시콘 숨기기` — 이미지·움짤·영상·텍스트콘을 포함한 모든 디시콘을 숨깁니다.
- `텍스트콘 숨기기` — 다른 디시콘은 그대로 두고 글자로 된 텍스트콘만 숨깁니다. 기본값은 꺼짐입니다.
- `비회원 게시물과 댓글 숨기기`
- `게임메카 글/댓글 숨기기`
- `댓글돌이 광고 숨기기`
- `운영자 광고, 설문 글 숨기기`
...
- `활동이 적은 회원 차단` — 작성 글·댓글 수가 설정한 기준보다 적은 회원의 게시글과 댓글을 숨깁니다. 회원 UID에만 적용합니다.
- `숨길 디시콘 선택하기` — `이 디시콘만 차단` 또는 `이 디시콘 그룹 전체 차단`으로 원하는 디시콘만 골라 차단합니다.

### 그 밖의 편의 기능

- `이용자 메모`
- `자동 새로고침` — 기본 간격은 60초이며, 필요에 따라 10초부터 10분까지 설정할 수 있습니다.
- `컴팩트 모드`
- `닉네임 옆 회원 ID 표시`
- `글꼴 설정`
- `메인 페이지 영역 숨김`
...
| 이미지 차단을 켰는데도 일부 이미지가 그대로 보인다 | `이미지 차단`을 이미지를 접어 숨기는 방식으로 바꾸고, `활동이 적은 회원 차단`을 함께 보강 | `7.3.32.2026` |
