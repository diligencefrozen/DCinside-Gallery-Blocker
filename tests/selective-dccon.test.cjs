...
test('textcon can be individually blocked with a stable local fingerprint', async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => getComputedStyle(document.querySelector('#row-2')).display === 'none');
  } finally {
    await page.close();
  }
});
...
test('textcon group block uses package_idx and also hides different textcons from the same package', async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => getComputedStyle(document.querySelector('#group-row-2')).display === 'none');
  } finally {
    await page.close();
  }
});
